...
export async function updateSession(request: NextRequest) {
  ...
  try {
    ...
    console.error('Supabase initialization/getUser error in proxy:', err)
  }
  ...
  if (!user && isProtectedRoute) {
    // Sem usuário logado, tentar acessar dashboard -> Redireciona pro Login
  ...
  if (user && isAuthRoute) {
    // Usuário logado tentando acessar login/cadastro -> Redireciona pro Dashboard
    const url = request.nextUrl.clone()
