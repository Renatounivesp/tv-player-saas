...
async function test() {
  ...
  const { data: authData, error: authError } = await supabase.auth.signInWithPassword({
    email: 'teste1779661632535@gmail.com', // from my previous test
  ...
  if (clinicsError || !clinics || clinics.length === 0) {
    console.error('Clinics error or empty:', clinicsError)
    return
  }
  
  const clinicId = clinics[0].id;
  
  console.log('Testing insert to slides for clinic:', clinicId);
  const { data, error } = await supabase.from('slides').insert([{
    clinic_id: clinicId,
    type: 'text',
    text_content: 'Test Slide',
    duration_seconds: 10,
    order_index: 0,
    is_active: true
  }]).select()
